...
for key in self_embedding.keys():
    di[key]=len(di.keys())
re_di=dict(zip(di.values(),di.keys()))
res=[]
for i in t.numpy():
    res.append(re_di[i])
print(res)
...
with open("./data/SST-cla/glove.txt") as f:
    ...
    for line in tqdm(lines):
        ...
        dic[data]=len(dic.keys())
re_di=dict(zip(dic.values(),dic.keys()))
for i in t.numpy():
    res.append(re_di[i])
print(res)
dic={}